...
abstract class   MY_Controller extends CI_Controller
{
    ...
    protected function update_post_data_by_id($m, $id, $ext_data = array())
    {
        ...
        if (!empty($HTTP_POST_FILES)) {
            $keys = array_keys($HTTP_POST_FILES);
            foreach ($keys as $value) {
                $updateData[$value] = $this->upload_file_temdir($value);
            }
        }
        if (!empty($updateData)) {
            $affected_rows = $m->update_data(array_merge($updateData, $ext_data));
    ...
    protected function insert_post_data($m, $ext_data = array())
    {
        ...
        if (!empty($HTTP_POST_FILES)) {
            $keys = array_keys($HTTP_POST_FILES);
            foreach ($keys as $value) {
                $insertData[$value] = $this->upload_file_temdir($value);
            }
        }
    ...
    public function ilist()
    {
        ...
        //  $datalist['pagination'] =  $this->create_page_html();
        $datalist['page_title'] = $this->subject . "列表";
        $template = file_exists(VIEWPATH . 'admin/' . $this->router->class . '/list' . '.php') ? 'admin/' . $this->router->class . '/list' : "common/list";
        $this->view($template, $datalist);
    }
     ...
     * @param $name
     */
    public function upload_file_temdir($name)
    {
